...
Player.prototype.getArtists = function (callback) {
  this.browse("A:ARTIST", "0", null, function (success, result) {
    if (!success) {
      callback(false);
      return;
    }
...
Player.prototype.getRadios = function (callback) {
    this.browse("R:", "0", "", function (success, result) {
    if (!success) {
      callback(false);
      return;
    }
...
/*
Player.prototype.getAlbums = function (callback, artist) {
  this.browse("A:ALBUM", "0", null, function (success, result) {
    if (!success) {
      callback(false);
      return;
    }
    callback(true, result.items, artist);
  });
}

Player.prototype.getAlbumTracks = function (callback, album) {
  this.browse("A:ALBUM", "0", null, function (success, result) {
    if (!success) {
      callback(false);
      return;
...
Player.prototype.getTrack = function (track, callback) {
  this.browse("A:TRACKS:"+track, "0", "", function (success, result) {
    if (!success) {
      callback(false);
      return;
    }
...
Player.prototype.clearQueue = function (startIndex, requestedCount, callback) {
    this.removeAllTracksFromQueue(function (success) {
          if (!success) {
...
Player.prototype.replaceWithTrack = function (trackTitle, trackURI, callback) {
  var player = this;
  if (!callback)
    callback = function () {};

  player.getTrack(trackTitle, function (success, tracks) {
      tracks.forEach(function (item) {
      if (item.uri == trackURI) {
        console.log("replaceWithTrack found it", item)
        ...
        player.removeAllTracksFromQueue(function (success) {
          ...
          player.addURIToQueue(item.uri, item.metaData, function (success) {
            ...
            var queueURI = "x-rincon-queue:" + player.uuid + "#0";
            player.setAVTransportURI(queueURI, item.metadata, function (success) {
              callback(success);
            });
          });
        });
...
Player.prototype.replaceWithListOfTracks = function (tracksTitles, tracksURIs, callback) {
    var player = this;
//    var i = 0;
    if (!callback)
	callback = function () {};

    var titles = tracksTitles.split(",");
    var uris = tracksURIs.split(",");
    titles.forEach (function (trackTitle) {
	console.log("replaceWithListOfTracks title=", trackTitle);
	player.getTrack(trackTitle, function (success, tracks) {
	    console.log("replaceWithListOfTracks tracks=", tracks)
	    tracks.forEach(function (item) {
		var j = 0;
		var trackURI = '';
		for (var j = 0; j < uris.length; j++) {
		    if (trackTitle == titles[j]) {
			trackURI = uris[j];
			break;
		    }
		}
		...
		//var trackURI = uris[i];
		console.log("replaceWithListOfTracks title=", trackTitle, ", uri=", trackURI,", item.uri=", item.uri);//,", i=",i)
		if (item.uri == trackURI) {
		    console.log("replaceWithListOfTracks found it", item)
		    /*
		      player.removeAllTracksFromQueue(function (success) {
		      if (!success) {
		      console.error("error when removing tracks");
...
//	    i++;
    ...
    /*
      player.setAVTransportURI(track.title, null, function (success) {
      callback(success);
      });
      ...
      player.addURIToQueue(track.title, "", function (success) {
      if (!success) {
      console.error("problem adding URI to queue");
      callback(false);
      return;
...
Player.prototype.replaceWithMP3File = function (trackTitle, trackURI, callback) {
  var player = this;
  if (!callback)
    callback = function () {};
    ...
    player.removeAllTracksFromQueue(function (success) {
        ...
        player.addURIToQueue(trackURI, '', function (success) {
            if (!success) {
              console.error("problem adding URI to queue");
              callback(false);
              return;
